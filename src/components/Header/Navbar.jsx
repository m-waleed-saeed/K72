...
const Navbar = () => {
    ...
    return (
        <div className='flex items-start justify-between fixed top-0 w-full z-4'>
            ...
            <div onClick={() => setNavOpen(true)} onMouseEnter={() => { setIsHover(true); navGreenRef.current.style.height = '100%' }} onMouseLeave={() => { setIsHover(false); navGreenRef.current.style.height = '0%' }} className='relative h-10 w-[17vw] bg-black cursor-pointer'>
                <div ref={navGreenRef} className='absolute top-0 h-0 w-full bg-primary transition-all'></div>
                <div className='relative h-full flex flex-col justify-center items-end gap-1.5 px-8'>
                    <div className={`w-16 h-0.5 transition-colors duration-300 ${isHover ? "bg-black" : "bg-white"}`} ></div>
                    <div className={`w-9 h-0.5 transition-colors duration-300 ${isHover ? "bg-black" : "bg-white"}`} ></div>
                </div>
            </div>
        </div >
    )
